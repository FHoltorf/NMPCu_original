...
nfe = 24
tf_bounds = [25.0*24.0/nfe, 25.0*24.0/nfe]
...
for i in range(1,nfe):
    ...
    e.set_regularization_weights(R_w=0.0,Q_w=0.0,K_w=0.0) # R_w controls, Q_w states, K_w control displacement
...
try:
    ...
    plt.figure(l)
    dimension = 3 # dimension n of the n x n matrix, n = #DoF
    rhs_confidence = chi2.isf(1.0-0.95,dimension) # 0.1**2*5% measurment noise, 95% confidence level, dimension = #DoF
    rows = {}
    for r in range(1,24):
...
# in case 2 parameters are used        
# transform in polar coordinates for simple plots
...
# plot confidence intervals incl. current estimate
