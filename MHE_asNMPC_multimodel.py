...
for i in range(1,nfe):
    ...
    #if not(e.plant_trajectory[i,'solstat'] == ['ok','optimal']):
    #    break
...
plots = [('Y',(1,)),('PO',(1,)),('PO_fed',(1,)),('W',(1,)),('MY',(1,))]
for p in plots: 
...
diff_norm = []
#for i in range(1,k):
#    state_offset_norm.append(sum(state_offset[i][k]**2 for k in state_offset[i])**0.5)
#    before_norm.append(sum(before[i][k]**2 for k in before[i]))
#    after_norm.append(sum(after[i][k]**2 for k in after[i]))
#    diff_norm.append(sum(diff[i][k]**2 for k in diff[i])**0.5)
    
#plt.figure(l+1)
#plt.plot(before_norm, label = "before")
#plt.plot(after_norm, label = "after')
#plt.plot(diff_norm, label = 'control')
#plt.plot(state_offset_norm, label = 'state')
#plt.legend()
