...
class SemiBatchPolymerization(ConcreteModel):
    def __init__(self, nfe, ncp, **kwargs):   
        ...
        self.F_0 = Var(initialize=3)
        self.dT_cw_dt_0 = Var(initialize=0)
        
        self.K_F_T = Var(self.fe_t, initialize=1)
        self.K_F_Y = Var(self.fe_t, initialize=1)
        self.K_F_PO = Var(self.fe_t, initialize=1)
 #       self.K_F_W = Var(self.fe_t, initialize=1)
        self.K_F_MY = Var(self.fe_t, initialize=1)
        self.K_F_MX0 = Var(self.fe_t, initialize=1)
        self.K_F_MX1 = Var(self.fe_t, initialize=1)
        
        self.K_cw_T = Var(self.fe_t, initialize=1)
        self.K_cw_Y = Var(self.fe_t, initialize=1)
        self.K_cw_PO = Var(self.fe_t, initialize=1)
#        self.K_cw_W = Var(self.fe_t, initialize=1)
        self.K_cw_MY = Var(self.fe_t, initialize=1)
        self.K_cw_MX0 = Var(self.fe_t, initialize=1)
        self.K_cw_MX1 = Var(self.fe_t, initialize=1)
        
        self.u1 = Var(self.fe_t, initialize=1.0,bounds=(0.0,None))
        self.u2 = Var(self.fe_t, initialize=1.0,bounds=(0.0,None))
        self.u3 = Var(self.fe_t, initialize=1.0,bounds=(0.0,None))
        self.u4 = Var(self.fe_t, initialize=1.0,bounds=(0.0,None))
        self.u5 = Var(self.fe_t, initialize=1.0,bounds=(0.0,None))
        self.u6 = Var(self.fe_t, initialize=1.0,bounds=(0.0,None))
        self.u7 = Var(self.fe_t, initialize=1.0,bounds=(0.0,None))
        self.u8 = Var(self.fe_t, initialize=1.0,bounds=(0.0,None))
        self.u9 = Var(self.fe_t, initialize=1.0,bounds=(0.0,None))
        self.u10 = Var(self.fe_t, initialize=1.0,bounds=(0.0,None))
        self.u11 = Var(self.fe_t, initialize=1.0,bounds=(0.0,None))
        self.u12 = Var(self.fe_t, initialize=1.0,bounds=(0.0,None))
        self.u13 = Var(self.fe_t, initialize=1.0,bounds=(0.0,None))
        self.u14 = Var(self.fe_t, initialize=1.0,bounds=(0.0,None))
...
#        
        ...
        self.u1_e = Expression(self.fe_t, rule = lambda self, i: self.K_F_T[i])
        self.u2_e = Expression(self.fe_t, rule = lambda self, i: self.K_F_PO[i])
        self.u3_e = Expression(self.fe_t, rule = lambda self, i: self.K_F_Y[i])
        self.u4_e = Expression(self.fe_t, rule = lambda self, i: self.K_F_MY[i])
        self.u5_e = Expression(self.fe_t, rule = lambda self, i: self.K_F_MX0[i])
        self.u6_e = Expression(self.fe_t, rule = lambda self, i: self.K_F_MX1[i])
        self.u7_e = Expression(self.fe_t, rule = lambda self, i: self.K_cw_T[i])
        self.u8_e = Expression(self.fe_t, rule = lambda self, i: self.K_cw_PO[i])
        self.u9_e = Expression(self.fe_t, rule = lambda self, i: self.K_cw_Y[i])
        self.u10_e = Expression(self.fe_t, rule = lambda self, i: self.K_cw_MY[i])
        self.u11_e = Expression(self.fe_t, rule = lambda self, i: self.K_cw_MX0[i])
        self.u12_e = Expression(self.fe_t, rule = lambda self, i: self.K_cw_MX1[i])
        ...
        self.u3_c = Constraint(self.fe_t, rule = lambda self, i: self.u3_e[i] == self.u3[i])
        self.u4_c = Constraint(self.fe_t, rule = lambda self, i: self.u4_e[i] == self.u4[i])
        self.u5_c = Constraint(self.fe_t, rule = lambda self, i: self.u5_e[i] == self.u5[i])
        self.u6_c = Constraint(self.fe_t, rule = lambda self, i: self.u6_e[i] == self.u6[i])
        self.u7_c = Constraint(self.fe_t, rule = lambda self, i: self.u7_e[i] == self.u7[i])
        self.u8_c = Constraint(self.fe_t, rule = lambda self, i: self.u8_e[i] == self.u8[i])
        self.u9_c = Constraint(self.fe_t, rule = lambda self, i: self.u9_e[i] == self.u9[i])
        self.u10_c = Constraint(self.fe_t, rule = lambda self, i: self.u10_e[i] == self.u10[i])
        self.u11_c = Constraint(self.fe_t, rule = lambda self, i: self.u11_e[i] == self.u11[i])
        self.u12_c = Constraint(self.fe_t, rule = lambda self, i: self.u12_e[i] == self.u12[i])
        
        def _F_computation(self,i,s):
            if i == 1:
                return 0.0 == self.F[i] - self.F_0 - self.K_F_T[i]*(self.T[i,3,s]-self.T_ic[1]) \
                                                   - self.K_F_PO[i]*(self.PO[i,3,s]-self.PO_ic[1]) \
                                                   - self.K_F_Y[i]*(self.Y[i,3,s]-self.Y_ic[1]) \
                                                   - self.K_F_MY[i]*(self.MY[i,3,s]-self.MY_ic[1]) \
                                                   - self.K_F_MX0[i]*(self.MX[i,3,0,s]-self.MX_ic[0,1]) \
                                                   - self.K_F_MX1[i]*(self.MX[i,3,1,s]-self.MX_ic[1,1])
            else:
                return 0.0 == self.F[i] - self.F[i-1] - self.K_F_T[i]*(self.T[i,3,s]-self.T[i-1,3,s]) \
                                                   - self.K_F_PO[i]*(self.PO[i,3,s]-self.PO[i-1,3,s]) \
                                                   - self.K_F_Y[i]*(self.Y[i,3,s]-self.Y[i-1,3,s]) \
                                                   - self.K_F_MY[i]*(self.MY[i,3,s]-self.MY[i-1,3,s]) \
                                                   - self.K_F_MX0[i]*(self.MX[i,3,0,s]-self.MX[i-1,3,0,s]) \
                                                   - self.K_F_MX1[i]*(self.MX[i,3,1,s]-self.MX[i-1,3,1,s])
                                                   
        self.F_computation = Constraint(self.fe_t, self.s, rule=_F_computation)
        
        def _cw_computation(self,i,s):
            if i == 1:
                return 0.0 == self.dT_cw_dt[i] - self.dT_cw_dt_0 - self.K_cw_T[i]*(self.T[i,3,s]-self.T_ic[1]) \
                                                   - self.K_cw_PO[i]*(self.PO[i,3,s]-self.PO_ic[1]) \
                                                   - self.K_cw_Y[i]*(self.Y[i,3,s]-self.Y_ic[1]) \
                                                   - self.K_cw_MY[i]*(self.MY[i,3,s]-self.MY_ic[1]) \
                                                   - self.K_cw_MX0[i]*(self.MX[i,3,0,s]-self.MX_ic[0,1]) \
                                                   - self.K_cw_MX1[i]*(self.MX[i,3,1,s]-self.MX_ic[1,1])
            else:
                return 0.0 == self.dT_cw_dt[i] - self.dT_cw_dt[i-1] - self.K_cw_T[i]*(self.T[i,3,s]-self.T[i-1,3,s]) \
                                                   - self.K_cw_PO[i]*(self.PO[i,3,s]-self.PO[i-1,3,s]) \
                                                   - self.K_cw_Y[i]*(self.Y[i,3,s]-self.Y[i-1,3,s]) \
                                                   - self.K_cw_MY[i]*(self.MY[i,3,s]-self.MY[i-1,3,s]) \
                                                   - self.K_cw_MX0[i]*(self.MX[i,3,0,s]-self.MX[i-1,3,0,s]) \
                                                   - self.K_cw_MX1[i]*(self.MX[i,3,1,s]-self.MX[i-1,3,1,s])
                                                   
        self.cw_computation = Constraint(self.fe_t, self.s, rule=_cw_computation)
...
#        self.epc_mw.deactivate() 
    ...
    def clear_aux_bounds(self):
        keep_bounds = ['s_temp_b','s_T_min','s_T_max','s_mw','s_PO_ptg','s_unsat','s_mw','s_mw_ub','s_PO_fed','eps','eps_pc','tf','T_cw','k_l','F','dT_cw_dt'] 
