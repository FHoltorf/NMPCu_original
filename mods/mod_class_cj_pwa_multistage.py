...
class SemiBatchPolymerization_multistage(ConcreteModel):
    def __init__(self, nfe, ncp, **kwargs):   
        ...
        # heat transfer
        self.kA = Var(initialize=2200.0/self.Hrxn['p']*60/30.0) 
        self.kA.fix()
...
#        
            ...
            if (i,s) in self.scenario_tree:
                if j > 0:
                    ...
                    return self.dT_dt[i,j,s]*(self.m_tot[i,j,s]*self.m_tot_scale)*(self.bulk_cp_1 + self.bulk_cp_2*self.T[i,j,s]*self.T_scale) ==\
                                (self.Qr[i,j,s] - self.Qc[i,j,s] - self.monomer_cooling[i,j,s])*self.Hrxn['p']/self.T_scale
                else:
                    return Constraint.Skip
            else:
                return Constraint.Skip
        ...
        def _eobj(self):
            return 1.0/self.s_max * sum(sum(self.tf[i,s] for i in self.fe_t if (i,s) in self.scenario_tree) for s in self.s) \
                    ...
                    + sum(sum(sum(sum(self.eps_pc[i,j,k,s] for i in self.fe_t) for j in self.cp) for k in self.pc) for s in self.s if (i,s) in self.scenario_tree))
    ...
    def create_bounds(self):
        for s in self.s:
            for i in self.fe_t:
                ...
                self.u1[i,s].setlb(-100.0/self.T_scale/10.0)
                self.dT_cw_dt[i,s].setub(100.0/self.T_scale/10.0)
                self.u1[i,s].setub(100.0/self.T_scale/10.0)
