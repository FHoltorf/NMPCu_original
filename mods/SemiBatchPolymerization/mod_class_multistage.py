...
class SemiBatchPolymerization_multistage(ConcreteModel):
    def __init__(self, nfe, ncp, **kwargs):   
        ...
        self.F = Var(self.fe_t, self.s, initialize=1.0,bounds=(0.0,None)) # monomer fe_ted
        ...
        self.Y = Var(self.fe_t,self.cp, self.s, initialize=self.Y_ic, bounds=(0.0,None)) # unsaturated by-products
        ...
        def _pc_temp_b(self,i,j,s):
            if (i,s) in self.scenario_tree:
                if j == 0:
                    ...
                    return 0.0 == (self.T_safety + self.Tb) - self.Tad[i,j,s]*self.Tad_scale - self.s_temp_b[i,j,s] + self.eps_pc[i,j,1,s] - self.xi_temp_b[i,j,s]
            else:
                return Constraint.Skip
        ...
        def _epc_unsat(self,i,j,s):
            if (i,s) in self.scenario_tree:
                if i == nfe and j == ncp:
                    return  0.0 == (self.unsat_value*self.m_tot[i,j,s]*self.m_tot_scale - 1000.0*(self.MY[i,j,s]*self.MY0_scale + self.Y[i,j,s]*self.Y_scale) + self.eps[2,s] - self.s_unsat[s]*100.0 - self.xi_unsat[s]) 
                else:
                    return Constraint.Skip
            else:
                return Constraint.Skip
        ...
        def _epc_mw_ub(self,i,j,s):
            if (i,s) in self.scenario_tree:
                if i == nfe and j == ncp:
                    return 0.0 == self.MX[nfe,ncp,1,s]*self.MX1_scale - (self.molecular_weight_max + self.molecular_weight - self.mw_PG)/self.mw_PO/self.num_OH*self.MX[nfe,ncp,0,s]*self.MX0_scale - self.eps[4,s] + self.s_mw_ub[s] + self.xi_mw_ub[s]
                else:
                    return Constraint.Skip
            else:
                return Constraint.Skip
...
#        self.epc_mw.deactivate() 
    ...
    def par_to_var(self):
        ...
        self.Hrxn_aux['p'].setub(1.5)
        self.kA.setlb(0.5*0.143403441683)
        self.kA.setub(1.5*0.143403441683)
        self.n_KOH.setlb(0.5*2.70005346641)
        self.n_KOH.setub(1.5*2.70005346641)
    ...
    def create_output_relations(self):
        ...
        for index in self.MW.index_set():
            ...
            s = index[2]
            try:
                self.MW[index].value = (self.MX[i,sj,1,s].value*self.MX1_scale*self.mw_PO*self.num_OH/(self.MX[i,j,0,s].value*self.MX0_scale) + self.mw_PG)/self.MW_scale 
            except:
                self.MW[index].value = 1.0
    ...
    def create_bounds(self):
        for s in self.s:
            for i in self.fe_t:
                ...
                self.tf[i,s].setub(min(30.0,30.0*24.0/self.nfe))
        ...
        for i in self.fe_t:
            for s in self.s:
                for j in self.cp:
                    ...
                    self.Vi[i,j,s].setlb(0.0)
                    self.Vi[i,j,s].setub(None)
                    

    def clear_aux_bounds(self):
        keep_bounds = ['s_tf','s_temp_b','s_T_min','s_T_max','s_mw','s_PO_ptg','s_unsat','s_mw','s_mw_ub','s_PO_fed','eps','eps_pc','u1','u2','tf','k_l'] 
    ...
    def bounding(self,bounds,index):
        out = False 
        for j in range(1,4):
            i = index[0]
            s = index[1]
            T_ub = bounds[j][('T',())][1]*self.T_scale
            PO_ub = bounds[j][('PO',())][1]*self.PO_scale
            if T_ub - (self.T_max.value+self.Tb.value) > 0:
                self.xi_T_max[i,j,s] = self.xi_T_max[i,j,s].value + T_ub - (self.T_max.value+self.Tb.value)
                out = True
            
            int_T_ub = self.bulk_cp_1.value*T_ub + self.bulk_cp_2.value*(T_ub)**2/2.0
            int_Tad_ub = int_T_ub + (PO_ub*self.Hrxn['p'].value*self.Hrxn_aux['p'].value*self.p_Hrxn_aux['p',i,s].value)/(self.m_tot[i,j,s].value*self.m_tot_scale)
            T_ad_ub = -self.bulk_cp_1.value/self.bulk_cp_2.value + np.sqrt((self.bulk_cp_1.value/self.bulk_cp_2.value)**2 + 2*int_Tad_ub/self.bulk_cp_2.value)
            if T_ad_ub - (self.T_safety.value+self.Tb.value) > 0:
                self.xi_temp_b[i,j,s] = self.xi_temp_b[i,j,s].value + T_ad_ub - (self.T_safety.value+self.Tb.value) 
                out = True
        return out
...
#m.Tad_ic = Var(initialize=3.7315)
#m.plot_profiles(var_list=['W', 'X', 'm_tot','MY','PO', 'T_cw', 'T', 'Tad'],control_list=['F','D'])