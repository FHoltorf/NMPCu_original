...
#        self.D_scale = 1.0
        ...
        self.A.fix()
        self.p_A = Var(self.fe_t, self.r, initialize=1.0, bounds=(0.0,None)) # this makes sense to be time-variant
        self.p_A.fix()
        ...
        self.Hrxn_aux.fix()
        self.p_Hrxn_aux = Var(self.fe_t, self.r, initialize=1.0, bounds=(0.0,None)) # this makes sense to be time-variant
        self.p_Hrxn_aux.fix()
        ...
        # heat transfer
        self.kA = Var(initialize=2200.0/self.Hrxn['p']*60.0/10.0) # /20 works fine, means following T_cw [kW/K]
        self.kA.fix()
        self.p_kA = Var(self.fe_t, initialize=1.0, bounds=(0.0,None)) # this makes sense to be time-variant
        self.p_kA.fix()
        ...
        self.F = Var(self.fe_t, initialize=1.0,bounds=(0.0,None)) # monomer fe_ted
        self.D = Var(self.fe_t, initialize=0.1,bounds=(0.0,None))
        self.ch = Var(self.fe_t, self.cp, initialize=0.0)
        
        self.u1 = Var(self.fe_t, initialize=1.0,bounds=(0.0,None))
        self.u2 = Var(self.fe_t, initialize=1.0,bounds=(0.0,None))
        
        # differential variables
        self.T_cw = Var(self.fe_t, self.cp, initialize=self.T_cw_ic, bounds=(0.0,None)) # cooling temperature
        self.dT_cw_dt = Var(self.fe_t, self.cp, initialize=0.0)
        
        self.T = Var(self.fe_t, self.cp, initialize=386.0/self.T_scale) # temperature
        ...
        # algebraic variables
        self.Vi = Var(self.fe_t,self.cp, bounds=(0,1e10))
        self.G = Var(self.fe_t,self.cp)
        self.U = Var(self.fe_t,self.cp) 
        ...
        def _ode_T(self,i,j):    
            if j > 0:
                return self.T_scale*self.dT_dt[i,j]*(self.m_tot[i,j]*self.m_tot_scale*(self.bulk_cp_1 + self.bulk_cp_2*self.T[i,j]*self.T_scale))==\
                           self.Hrxn['p']*(self.Qr[i,j] - self.Qc[i,j] - self.F[i]*self.tf*self.fe_dist[i]*self.mw_PO*self.monomer_cooling[i,j]*self.monomer_cooling_scale)
            else:
                return Constraint.Skip
...
#        self.epc_mw.deactivate()
    ...
    def e_state_relation(self):
        # uses implicit assumption of Radau nodes
    ...
    def clear_aux_bounds(self):
        keep_bounds = ['s_temp_b','s_T_min','s_T_max','s_mw','s_PO_ptg','s_unsat','s_mw','s_mw_ub','s_PO_fed','eps','eps_pc','u1','u2','tf','k_l','T','T_cw','p_A','p_Hrxn_aux','p_kA']
    ...
    def create_bounds(self):
        ...
        for i in self.fe_t:
            self.D[i].setlb(0.0)
            self.u1[i].setlb(0.0)
            self.D[i].setub(0.4/self.D_scale)
            self.u1[i].setub(0.4/self.D_scale)
            self.F[i].setlb(0.0)
            self.u2[i].setlb(0.0)
            self.F[i].setub(3.0) 
            self.u2[i].setub(3.0)
            for j in self.cp:
                ...
                self.T_cw[i,j].setub((200.0 + self.Tb)/self.T_scale)
                self.T[i,j].setlb((70.0 + self.Tb)/self.T_scale)
                self.T[i,j].setub((200.0 + self.Tb)/self.T_scale)
                self.int_T[i,j].setlb(0.5*(1.1*(100+self.Tb) + 2.72*(100+self.Tb)**2/2000)/self.int_T_scale)
                self.int_T[i,j].setub(1.5*(1.1*(170+self.Tb) + 2.72*(170+self.Tb)**2/2000)/self.int_T_scale)
                self.Vi[i,j].setlb(0.5/self.Vi_scale*(1e3)/((self.m_KOH + self.m_PG + self.m_PO + self.m_H2O)*(1 + 0.0007576*((170+self.Tb)-298.15))))
                self.Vi[i,j].setub(1.5/self.Vi_scale*(1e3)/((self.m_KOH + self.m_PG + self.m_H2O)*(1 + 0.0007576*((100+self.Tb)-298.15))))
...
#e.create_bounds()
#e.clear_aux_bounds()
##e.A['p'] = 0.86*e.A['p'].value
##e.A['i'] = 1.1*e.A['i'].value
##e.kA = 1.02*e.kA.value
##e.T_icc.deactivate()
...
#tf = resource.getrusage(resource.RUSAGE_CHILDREN)
#e.plot_profiles([e.PO,e.W,e.Y,e.PO_fed,e.MY,e.T,e.T_cw,e.F,e.dT_cw_dt,e.Tad,e.D])
