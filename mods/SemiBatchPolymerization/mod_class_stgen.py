...
class SemiBatchPolymerization_multistage(ConcreteModel):
    def __init__(self, nfe, ncp, **kwargs):   
        ...
        self.p_T_cw_ic = Var(self.sg_ic, self.s, initialize=1.0)
        self.p_T_cw_ic_par = Param(self.sg_ic, self.s, initialize=1.0, mutable=True)
        ...
        # heat transfer
        self.kA = Var(initialize=2200.0/self.Hrxn['p']*60/10.0) 
        self.kA.fix()
        ...
        self.T_ic = Param(initialize=383.15/self.T_scale, mutable=True)#
        self.T_cw_ic = Param(initialize = 383.15/self.T_scale, mutable=True) 
        ...
        self.D = Var(self.fe_t, self.s, initialize=0.1, bounds=(0.0,None))
        self.u1 = Var(self.fe_t, self.s, initialize=397.0/self.T_scale)
        self.u2 = Var(self.fe_t, self.s, initialize=1,bounds=(0.0,None))
        
        # differential variables
        self.T_cw = Var(self.fe_t, self.cp, self.s, initialize=363.15/self.T_scale, bounds=(0.0,None)) # cooling temperature
        self.dT_cw_dt = Var(self.fe_t, self.cp, self.s, initialize=0.0)
        ...
        def _continuity_W(self,i,s):#cont_W
            if (i+1,s) in self.scenario_tree:
                if i < nfe and nfe > 1:
                    ...
                    p = self.p_W_ic[2,s] if i == 1 else 1.0
        ...
        def _continuity_PO(self,i,s):#cont_W
            if (i+1,s) in self.scenario_tree:
                if i < nfe and nfe > 1:
                    parent_s = self.scenario_tree[(i+1,s)][1] # parent scenario
                    p = self.p_PO_ic[2,s] if i == 1 else 1.0
        ...
        def _continuity_MX(self,i,o,s):#cont_MX
            if (i+1,s) in self.scenario_tree:
                if i < nfe and nfe > 1:
                    parent_s = self.scenario_tree[(i+1,s)][1] # parent scenario
                    p = self.p_MX_ic[o,2,s] if i == 1 else 1.0
        ...
        def _continuity_Y(self,i,s):#cont_Y
            if (i+1,s) in self.scenario_tree:
                if i < nfe and nfe > 1:
                    parent_s = self.scenario_tree[(i+1,s)][1] # parent scenario
                    p = self.p_Y_ic[2,s] if i == 1 else 1.0
        ...
        def _continuity_MY(self,i,s):#cont_MY
            if (i+1,s) in self.scenario_tree:
                if i < nfe and nfe > 1:
                    parent_s = self.scenario_tree[(i+1,s)][1] # parent scenario
                    p = self.p_MY_ic[2,s] if i == 1 else 1.0
        ...
        def _ode_T_cw(self,i,j,s):   
            if (i,s) in self.scenario_tree:
                if j > 0:
                    ...
                    return self.dT_cw_dt[i,j,s] == (self.D[i,s]*self.tf[i,s]*self.fe_dist[i]*(self.T_cw_in*self.T_scale - self.T_cw[i,j,s]*self.T_scale) + self.Qc[i,j,s]*self.Hrxn['p']/(self.m_cw*self.cp_cw))/self.T_scale
                else:
                    return Constraint.Skip
            else:
                 return Constraint.Skip
    
        self.de_T_cw = Constraint(self.fe_t, self.cp, self.s, rule=_ode_T_cw)                          

        def _collocation_T_cw(self,i,j,s):
            if (i,s) in self.scenario_tree:
                if j > 0:
                    return self.dT_cw_dt[i,j,s] == sum(self.ldot_t[j, k] * self.T_cw[i, k, s] for k in self.cp)
                else:   
                    return Constraint.Skip
            else:
                return Constraint.Skip
        ...
        def _continuity_T_cw(self,i,s):
            if (i+1,s) in self.scenario_tree:
                if i < nfe and nfe > 1:
                    parent_s = self.scenario_tree[(i+1,s)][1] # parent scenario
                    p = self.p_T_cw_ic[2,s] if i == 1 else 1.0
                    return self.T_cw[i+1,0,s] - sum(self.l1_t[j] * self.T_cw[i, j, parent_s] for j in self.cp) * p
                else:
                    return Expression.Skip 
            else:
                return Expression.Skip
        ...
        def _init_T_cw(self,s):
            if (1,s) in self.scenario_tree:
                if s == 1:
                    ...
                    return self.T_cw[1,0,s] - self.T_cw[1,0,1]*self.p_T_cw_ic[1,s]
            else:
                return Constraint.Skip
        ...
        def _continuity_T(self, i, s): 
            if (i+1,s) in self.scenario_tree:
                if i < nfe and nfe > 1:
                    parent_s = self.scenario_tree[(i+1,s)][1] # parent scenario
                    p = self.p_T_ic[2,s] if i == 1 else 1.0
        ...
        def _rxn_rate_r_a(self,i,j,r,s):
            if (i,s) in self.scenario_tree:
                k = i if i < self.nr + 2 else self.nr + 1
                if j > 0:
                    return 0.0 == (self.T[i,j,s]*self.T_scale*log(self.p_A[r,k,s]*self.A[r]*self.scale*60*1000) - self.Ea[r]/self.Rg - self.T[i,j,s]*self.T_scale*self.k_l[i,j,r,s])
                else:
                    return Constraint.Skip
            else:
                return Constraint.Skip
        ...
        def _Q_out(self,i,j,s):
            if (i,s) in self.scenario_tree:
                k = i if i < self.nr + 2 else self.nr + 1
                if j > 0:
                    return self.Qc[i,j,s] == self.kA * self.p_kA[k,s] * self.tf[i,s] * self.fe_dist[i] * (self.T[i,j,s]*self.T_scale - self.T_cw[i,j,s]*self.T_scale)
                else:
                    return Constraint.Skip
            else:
                return Constraint.Skip
        ...
        def _epc_PO_ptg(self,i,j,s):
            if (i,s) in self.scenario_tree:
                if i == nfe and j == ncp:
                    return  0.0 == self.unreacted_PO*1e-6*self.m_tot[i,j,s]*self.m_tot_scale - self.PO[i,j,s]*self.PO_scale*self.mw_PO + self.eps[1,s]*0.1 - self.s_PO_ptg[s]*0.1
                else:
                    return Constraint.Skip
            else:
                return Constraint.Skip
        ...
        self.u1_e = Expression(self.fe_t, self.s, rule = lambda self, i, s: self.D[i,s] if (i,s) in self.scenario_tree else Expression.Skip)
        ...
        self.s_tf = Var(self.fe_t, self.s, initialize=0.0, bounds=(0.0,None))
        ...
        self.dummy_constraint_p_T_cw_ic = Constraint(self.sg_ic, self.s, rule = lambda self,i,s: 0.0 == - self.p_T_cw_ic[i,s] + self.p_T_cw_ic_par[i,s])
        ...
        self.s_theta = Var(initialize=0.0, bounds=(0.0,None))
        def _epi(self,s):
            total_time = self.tf[self.nfe,s]
            for i in range(2,self.nfe+1):
                parent_node = self.scenario_tree[(i,s)][0:2]
                total_time += self.tf[parent_node]
            return 0.0 == self.theta - total_time - self.s_theta
...
#        self.epc_mw.deactivate()
    ...
    def clear_aux_bounds(self):
        keep_bounds = ['s_theta','s_tf','s_temp_b','s_T_min','s_T_max','s_mw','s_PO_ptg','s_unsat','s_mw','s_mw_ub','s_PO_fed','eps','eps_pc','u1','u2','tf','k_l'] 
    ...
    def create_bounds(self):
        for s in self.s:
            for i in self.fe_t:
                ...
                self.u1[i,s].setlb(0.0)
                self.D[i,s].setlb(0.0)
                self.u1[i,s].setub(0.4)
                self.D[i,s].setub(0.2)
    ...
    def initialize_element_by_element(self):
        ...
        for fe_t in self.fe_t:
            ...
            try:
                ...
                m_aux.T_cw_ic = prevsol['T_cw',(1,3,1)]
                ...
                for i in m_aux.fe_t:
                    for j in m_aux.cp:
                        ...
                        m_aux.T_cw[i,j,1] = prevsol['T_cw',(1,3,1)]
