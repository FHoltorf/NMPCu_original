...
class SemiBatchPolymerization(ConcreteModel):
    def __init__(self, nfe, ncp, **kwargs):   
        ...
        self.r_A = Var(self.r,  initialize=1.0)
        self.r_A_par = Param(self.r,  initialize=1.0, mutable=True)
        self.r_Hrxn_aux = Var(self.r,  initialize=1.0)
        self.r_Hrxn_aux_par = Param(self.r,  initialize=1.0, mutable=True)
        self.r_kA = Var( initialize=1.0)
        self.r_kA_par = Param( initialize=1.0, mutable=True)
        self.r_n_KOH = Var( initialize=1.0)
        self.r_n_KOH_par = Param( initialize=1.0, mutable=True)
        ...
        self.p_Hrxn_aux.fix()
        self.p_kA = Var(self.fe_t,  initialize=1.0)
        self.p_kA.fix()
        ...
        # cooling jacket
        self.m_cw = Param(initialize=5e3)
        self.cp_cw = Param(initialize=4.18)
        ...
        self.mono_cp_1 = Param(initialize=0.92)  # [kJ/kg/K]
        self.mono_cp_2 = Param(initialize=8.87e-3) # [kJ/kmol/K^2]
        self.mono_cp_3 = Param(initialize=-3.10e-5) # [kJ/kmol/K^3]
        self.mono_cp_4 = Param(initialize=4.78e-8) # [kJ/kmol/K^4]
        ...
        # heat transfer
        self.kA = Var(initialize=2200.0/self.Hrxn['p']*60.0/10.0) # 100 [kW/K]
        self.kA.fix()
        
        # parameters for initializing differential variabales
        self.W_ic = Param( initialize=self.n_H2O/self.W_scale, mutable=True)
        self.r_W_ic = Var( initialize=1.0)
        self.r_W_ic_par = Param( initialize=1.0, mutable=True)
        
        self.PO_ic = Param( initialize=0, mutable=True)
        self.r_PO_ic = Var( initialize=1.0)
        self.r_PO_ic_par = Param( initialize=1.0, mutable=True)
        ...
        self.r_X_ic = Var( initialize=1.0)
        self.r_X_ic_par = Param( initialize=1.0, mutable=True)
        
        self.Y_ic = Param( initialize=0, mutable=True)
        self.r_Y_ic = Var( initialize=1.0)
        self.r_Y_ic_par = Param( initialize=1.0, mutable=True)
        
        self.MY_ic = Param( initialize=0.0, mutable=True)
        self.r_MY_ic = Var( initialize=1.0)
        self.r_MY_ic_par = Param( initialize=1.0, mutable=True)
        
        self.MX_ic = Param(self.o,  initialize=0.0, mutable=True)
        self.r_MX_ic = Var(self.o,  initialize=1.0)
        self.r_MX_ic_par = Param(self.o,  initialize=1.0, mutable=True)
        
        self.PO_fed_ic = Param( initialize=0.0, mutable=True) 
        
        self.T_ic = Param( initialize=383.15/self.T_scale, mutable=True) #403.1
        self.r_T_ic = Var( initialize=1.0)
        self.r_T_ic_par = Param( initialize=1.0, mutable=True)
        
        self.T_cw_ic = Param( initialize=383.15/self.T_scale, mutable=True) #373.15
        self.T_cw_in = Param(initialize=353.15/self.T_scale, mutable=True)
        ...
        self.D = Var(self.fe_t, initialize=0.0)
        self.u1 = Var(self.fe_t, initialize=1.0,bounds=(0.0,None))
        self.u2 = Var(self.fe_t, initialize=1.0,bounds=(0.0,None))
        
        # differential variables
        self.T_cw = Var(self.fe_t, self.cp,  initialize=self.T_cw_ic, bounds=(0.0,None)) # cooling water temperature
        self.dT_cw_dt = Var(self.fe_t, self.cp,  initialize=0.0)
        ...
        self.PO_fed = Var(self.fe_t,self.cp,initialize=self.PO_fed_ic, bounds=(0.0,None)) # amount of PO fed
        self.dPO_fed_dt = Var(self.fe_t,self.cp)

        
        # reactions
        self.k_l = Var(self.fe_t, self.cp, self.r,  bounds=(-50,50)) # rate coefficients 
        ...
        self.heat_removal = Var(self.fe_t, self.cp,  initialize=1.121) # no idea
        ...
        self.s_heat_removal_a = Var(self.fe_t, self.cp,  initialize = 0, bounds=(0,None))
        ...
        def _ode_W(self,i,j):
            if j > 0:
                return self.dW_dt[i,j] == -self.kr[i,j,'a']*self.W[i,j]*self.PO[i,j]*self.PO_scale*self.Vi[i,j]*self.Vi_scale
            else:
                return Constraint.Skip
        ...
        def _init_W(self):#acW(self, t):
            return self.W[1,0] - self.W_ic*self.r_W_ic
        ...
        def _init_PO(self):#acW(self, t):
            return self.PO[1,0] - self.PO_ic*self.r_PO_ic
        ...
        def _init_MX(self,o):#acMX(self, t):
            return self.MX[1,0,o] - self.MX_ic[o]*self.r_MX_ic[o]
        ...
        def _init_Y(self):#acY(self, t):
            return self.Y[1,0] - self.Y_ic*self.r_Y_ic
        ...
        def _init_MY(self):#acMY(self, t):
            return self.MY[1,0] - self.MY_ic*self.r_MY_ic
        ...
        self.de_T_cw = Constraint(self.fe_t, self.cp,  rule=_ode_T_cw)       
        
        def _collocation_T_cw(self,i,j):
            if j > 0:
                # implicit systematic
                return self.dT_cw_dt[i,j] == sum(self.ldot_t[j, k] * self.T_cw[i, k] for k in self.cp)
            else:   
                return Constraint.Skip
        ...
        def _init_T(self):        
            return self.T[1, 0] - self.T_ic*self.r_T_ic
        ...
        def _rxn_rate_r_a(self,i,j,r):
            if j > 0:
                if r == 'p':
                    return 0.0 == (self.T[i,j]*self.T_scale*log(self.r_A[r]*self.p_A[i,r]*self.A[r]*60*1000) - self.Ea[r]/self.Rg - self.T[i,j]*self.T_scale*self.k_l[i,j,r])
                elif r== 'i':
                    return 0.0 == (self.T[i,j]*self.T_scale*log(self.r_A[r]*self.p_A[i,r]*self.A[r]*60*1000) - self.Ea[r]/self.Rg - self.T[i,j]*self.T_scale*self.k_l[i,j,r])
                else:
                    return 0.0 == (self.T[i,j]*self.T_scale*log(self.p_A[i,r]*self.A[r]*60*1000) - self.Ea[r]/self.Rg - self.T[i,j]*self.T_scale*self.k_l[i,j,r])
            else:    
                return Constraint.Skip
        ...
        def _Q_in(self,i,j):
            if j > 0: # normalized with delta_H_r
                return self.Qr[i,j] == ((self.kr[i,j,'i']-self.kr[i,j,'p'])*(self.G[i,j]*self.G_scale + self.U[i,j]*self.U_scale) + (self.kr[i,j,'p'] + self.kr[i,j,'t'])*self.n_KOH + self.kr[i,j,'a']*self.W[i,j]*self.W_scale)*self.PO[i,j]*self.PO_scale*self.Vi[i,j]*self.Vi_scale*self.Hrxn_aux['p']*self.r_Hrxn_aux['p']*self.p_Hrxn_aux[i,'p'] + self.dW_dt[i,j]*self.W_scale * self.Hrxn_aux['p']*self.r_Hrxn_aux['p']*self.p_Hrxn_aux[i,'p']
            else:
                return Constraint.Skip
        ...
        def _Q_out(self,i,j):
            if j > 0:
                return self.Qc[i,j] == self.kA*self.r_kA*self.p_kA[i]*self.tf*self.fe_dist[i] * (self.T[i,j]*self.T_scale - self.T_cw[i,j]*self.T_scale)
            else:
                return Constraint.Skip
        ...
        def _pc_temp_a(self,i,j):
            if j == 0:
                ...
                return 0.0 == self.m_tot[i,j]*self.m_tot_scale*(self.int_Tad[i,j]*self.int_Tad_scale - self.int_T[i,j]*self.int_T_scale) - self.PO[i,j]*self.PO_scale*self.Hrxn['p']*self.Hrxn_aux['p']*self.r_Hrxn_aux['p']*self.p_Hrxn_aux[i,'p']
        ...
        def _pc_temp_b(self,i,j):
            if j == 0:
                ...
                return 0.0 == (self.T_safety + self.Tb) - self.Tad[i,j]*self.Tad_scale - self.s_temp_b[i,j] - self.xi_temp_b[i,j] + self.eps_pc[i,j,1] 
        ...
        def _pc_heat_removal_c(self,i,j):
            if j == 0:
                ...
                return 0.0 == (self.mono_cp_1*(self.T[i,j]*self.T_scale - self.feed_temp) + self.mono_cp_2/2.0 *((self.T[i,j]*self.T_scale)**2.0 -self.feed_temp**2.0) + self.mono_cp_3/3.0*((self.T[i,j]*self.T_scale)**3.0 -self.feed_temp**3.0)+ self.mono_cp_4/4.0*((self.T[i,j]*self.T_scale)**4.0-self.feed_temp**4.0)) - self.monomer_cooling[i,j]*self.monomer_cooling_scale*self.Hrxn['p']  
        ...
        self.u1_e = Expression(self.fe_t, rule = lambda self, i: self.D[i])
        ...
        # dummy_constraints    
        self.dummy_constraint_r_A_p = Constraint( rule = lambda self: self.r_A['p'] == self.r_A_par['p'])
        self.dummy_constraint_r_A_i = Constraint( rule = lambda self: self.r_A['i'] == self.r_A_par['i'])
        self.dummy_constraint_r_Hrxn_aux_p = Constraint( rule = lambda self: self.r_Hrxn_aux['p'] == self.r_Hrxn_aux_par['p'])
        self.dummy_constraint_r_kA = Constraint( rule=lambda self: self.r_kA == self.r_kA_par)
        self.dummy_constraint_r_n_KOH = Constraint( rule=lambda self: self.r_n_KOH == self.r_n_KOH_par)
        
        self.dummy_constraint_r_T_ic = Constraint( rule=lambda self: self.r_T_ic == self.r_T_ic_par)
        self.dummy_constraint_r_W_ic = Constraint( rule=lambda self: self.r_W_ic == self.r_W_ic_par)
        self.dummy_constraint_r_PO_ic = Constraint( rule=lambda self: self.r_PO_ic == self.r_PO_ic_par)
        self.dummy_constraint_r_Y_ic = Constraint( rule=lambda self: self.r_Y_ic == self.r_Y_ic_par)
        self.dummy_constraint_r_MY_ic = Constraint( rule=lambda self: self.r_MY_ic == self.r_MY_ic_par)
        self.dummy_constraint_r_MX_ic_0 = Constraint( rule=lambda self: self.r_MX_ic[0] == self.r_MX_ic_par[0])
        self.dummy_constraint_r_MX_ic_1 = Constraint( rule=lambda self: self.r_MX_ic[1] == self.r_MX_ic_par[1])
        
        # objective
        def _eobj(self):
            return self.tf + self.rho*(sum(self.eps[i] for i in self.epc) \
                           + sum(sum(sum(self.eps_pc[i,j,k] for i in self.fe_t) for j in self.cp if j > 0) for k in self.pc))
#            return self.tf + self.rho*(sum(sum(self.eps[i] for i in self.epc) for s in self.s) \
#                                       + sum(sum(sum(sum(self.eps_pc[i,j,k] for i in self.fe_t) for j in self.cp if j > 0) for k in self.pc) for s in self.s)) \
#                                       + self.gamma*sum((self.MX[self.nfe,self.ncp,1]*self.MX1_scale/(self.MX[self.nfe,self.ncp,0]*self.MX0_scale)*self.mw_PO*self.num_OH + self.mw_PG - self.molecular_weight)**2 for s in self.s)
...
#        self.epc_mw.deactivate() 
    ...
    def create_output_relations(self):
        self.add_component('MW', Var(self.fe_t, self.cp, initialize=1.0, bounds=(0,None)))
    ...
    def create_bounds(self):
        ...
        for i in self.fe_t:
            ...
            self.u2[i].setub(3.0)
            for j in self.cp:    
                self.T_cw[i,j].setlb(298.15/self.T_scale)
                ...
                self.int_T[i,j].setub((1.1*(self.T_safety+self.Tb) + 2.72*(170+self.Tb)**2/2000)/self.int_T_scale)
    ...
    def clear_aux_bounds(self):
        keep_bounds = ['s_temp_b','s_T_min','s_T_max','s_mw','s_PO_ptg','s_unsat','s_mw','s_mw_ub','s_PO_fed','eps','eps_pc','u1','u2','tf','k_l','T_cw'] 
    ...
    def initialize_element_by_element(self):
        ...
        m_aux.F[1] = 1.26
        m_aux.D[1] = 0.2
        m_aux.tf = min(12.0*24.0/self.nfe,12.0)
        m_aux.F[1].fixed = True
        m_aux.D[1].fixed = True
        m_aux.tf.fixed = True
        ...
        for k in range(len(v)):
            if v[k].dim() > 1:
                ...
                for key in v[k].index_set():         
                    # continuous
                    i = key[0] # finite element index 
                    cp = key[1] # collocation point index
                    if cp != 0:
                        t.append((i+self.tau_i_t[cp])*self.tf.value)
