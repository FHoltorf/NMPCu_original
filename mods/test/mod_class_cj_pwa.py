...
#        self.epc_mw.deactivate()
    ...
    def enmpc(self):
        def _obj(self):
            return -self.PO_fed[self.nfe,self.ncp]*self.PO_fed_scale + self.rho*(sum(self.eps[i] for i in self.epc) + sum(sum(sum(self.eps_pc[i,j,k] for i in self.fe_t) for j in self.cp if j > 0) for k in self.pc))
        self.obj = Expression(rule=_obj)
        self.epc_PO_fed.deactivate()
        self.tf.fix()
        self.eobj.expr = self.obj       
