...
#        self.monomer_cooling_scale = 1#1e-2
        ...
        self.T_cw = Var(self.fe_t, self.cp, initialize=397.0/self.T_scale, bounds=(0.0,None))
        self.T_cw_init = Param(initialize=363.15/self.T_scale) 
...
#        
        ...
        # energy balance
        def _pwa_controls(self,i,j):
            if j > 0:
                return self.dT_cw_dt[i] == sum(self.ldot_t[j, k] * self.T_cw[i, k] for k in self.cp)/self.tf
            elif j == 0 and i < self.nfe:
                return self.T_cw[i+1,j] == sum(self.l1_t[j] * self.T_cw[i, j] for j in self.cp)
            else:   
                return Constraint.Skip
        
        self.pwa_controls = Constraint(self.fe_t, self.cp, rule=_pwa_controls)
        
        def _controls_init(self):
            return self.T_cw[1,0] == self.T_cw_init
        
        self.controls_init = Constraint(rule=_controls_init)
        ...
        def _ode_T(self,i,j):    
            if j > 0:
                ...
                return self.dT_dt[i,j]*(self.m_tot[i,j]*self.m_tot_scale)*(self.bulk_cp_1 + self.bulk_cp_2*self.T[i,j]*self.T_scale) ==\
                            (self.Q_R[i,j] - self.Q_C[i,j] - self.monomer_cooling[i,j])*self.Hrxn['p']/self.T_scale
        ...
        def _Q_out(self,i,j):
            if j > 0:
                return self.Q_C[i,j] == self.kA*self.tf*self.fe_dist[i]*self.Hrxn_aux['p'] * (self.T[i,j]*self.T_scale - self.T_cw[i,j]*self.T_scale)
            else:
                return Constraint.Skip
        ...
        self.T_max = Constraint(self.fe_t, self.cp, rule = _T_max)
    ...
    def clear_aux_bounds(self):
        keep_bounds = ['s_temp_b','s_heat_removal_a','s_mw','s_PO_ptg','s_unsat','s_mw','s_mw_ub','s_PO_fed','s_max','eps','eps_pc','F','u1','u2','tf'] 
        for var in self.component_objects(Var, active=True):
    ...
    def create_bounds(self):
        ...
        for i in self.fe_t:
            self.dT_cw_dt[i].setlb(-100.0/self.T_scale/10.0)
            self.u1[i].setlb(-100.0/self.T_scale/10.0)
            self.dT_cw_dt[i].setub(100.0/self.T_scale/10.0)
            self.u1[i].setub(100.0/self.T_scale/10.0)
            self.F[i].setlb(0.0)
            self.u2[i].setlb(0.0)
            self.F[i].setub(3.0) #5*self.n_PO/(3.0*60))
            self.u2[i].setub(3.0)
            for j in self.cp:
                self.T_cw[i,j].setlb((50 + self.Tb)/self.T_scale)
                self.T_cw[i,j].setub((170 + self.Tb)/self.T_scale)
