...
class SemiBatchPolymerization(ConcreteModel):
    def __init__(self, nfe, ncp, **kwargs):   
        ...
        self.T_cw_ic = Param(self.s, initialize=373.15/self.T_scale, mutable=True) #373.15
...
#        
        ...
        def _ode_T_cw(self,i,j,s):   
            if j > 0:
                return self.dT_cw_dt[i,j,s] == (self.D[i]*self.tf*self.fe_dist[i]*(self.T_cw_in*self.T_scale - self.T_cw[i,j,s]*self.T_scale) + self.Qc[i,j,s]*self.Hrxn['p']/(self.m_cw*self.cp_cw))/self.T_scale
            else:
                return Constraint.Skip
...
#        self.epc_mw.deactivate() 
    ...
    def initialize_element_by_element(self):
        ...
        for fe_t in self.fe_t:
            ...
            try:
                m_aux.T_ic[1] = prevsol['T',(1,3,1)]
                m_aux.T_cw_ic[1] = prevsol['T_cw',(1,3,1)]
