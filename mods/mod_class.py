...
#        self.monomer_cooling_scale = 1#1e-2
        ...
        self.PO_fed_ic = Param(initialize=0, mutable=True) 
...
#        
        ...
        def _ae_V(self,i,j):
            if j == 0:
                ...
                return 0.0 == (1e3 - self.Vi[i,j]*self.Vi_scale * self.m_tot[i,j]*self.m_tot_scale*(1+0.0007576*(self.T[i]*self.T_scale - 298.15)))
        ...
        def _pc_heat_a(self,i,j):
            if j == 0:
                ...
                return 0.0 == self.bulk_cp_1*(self.T[i]*self.T_scale) + self.bulk_cp_2*(self.T[i]*self.T_scale)**2/2.0 - self.int_T[i,j]*self.int_T_scale
        ...
        def _pc_heat_removal_a(self,i,j):
            if j == 0:
                return Constraint.Skip
            else:
        ...
        def _pc_heat_removal_b(self,i,j):
            if j == 0:
                return Constraint.Skip
            else:
        ...
        def _pc_heat_removal_c(self,i,j):
            if j == 0:
                ...
                return 0.0 == (self.mono_cp_1*(self.T[i]*self.T_scale - self.feed_temp) + self.mono_cp_2/2.0 *((self.T[i]*self.T_scale)**2.0 -self.feed_temp**2.0) + self.mono_cp_3/3.0*((self.T[i]*self.T_scale)**3.0 -self.feed_temp**3.0)+ self.mono_cp_4/4.0*((self.T[i]*self.T_scale)**4.0-self.feed_temp**4.0)) - self.monomer_cooling[i,j]*self.monomer_cooling_scale*self.Hrxn['p']  
...
#for i in range(1,24):
#    nfe_new = 24-i
#    m = SemiBatchPolymerization(nfe_new,3)
#    m.create_output_relations()
#    m.create_bounds()
#    m.clear_aux_bounds()
#    m.W_ic = prev_res['W',(1,3)]
#    m.PO_ic = prev_res['PO',(1,3)]
#    #m.m_tot_ic = prev_res['m_tot',(1,3)]
#    #m.X_ic = prev_res['X',(1,3)]
#    m.Y_ic = prev_res['Y',(1,3)]
#    m.PO_fed_ic = prev_res['PO_fed',(1,3)]
#    for o in m.o:    
#        m.MX_ic[o] = prev_res['MX',(1,3,o)]
#    # initial guess for next element
