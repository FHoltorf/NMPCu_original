...
#        self.monomer_cooling_scale = 1#1e-2
        ...
        self.T_cw = Var(self.fe_t, initialize=397.0/self.T_scale, bounds=(0.0,None))
        ...
        self.Q_R = Var(self.fe_t,self.cp, initialize=0.0)
        self.Q_C = Var(self.fe_t,self.cp, initialize=0.0)
        
        # define slack variables
            # path constraints
        self.s_temp_b = Var(self.fe_t, self.cp, initialize = 0, bounds=(0,None))
        self.s_T_max = Var(self.fe_t, self.cp, initialize= 0, bounds=(0,None))
        self.s_T_min = Var(self.fe_t, self.cp, initialize= 0, bounds=(0,None))
...
#            if j > 0:
#                return self.dm_tot_dt[i,j] == self.mw_PO*self.F[i]*self.tf*self.fe_dist[i]/self.m_tot_scale
#            else:
#                return Constraint.Skip
#        
...
#            if j > 0:
#                return self.dm_tot_dt[i, j] == \
#                       sum(self.ldot_t[j, k] * self.m_tot[i, k] for k in self.cp)
#            else:
#                return Constraint.Skip
#                
...
#            if i < nfe and nfe > 1:
#                return self.m_tot[i + 1, 0] - sum(self.l1_t[j] * self.m_tot[i, j] for j in self.cp)
#            else:
#                return Expression.Skip
#        
...
#        def _ode_X(self,i,j):
#            if j > 0:
#                return self.dX_dt[i,j] == (2*self.kr[i,j,'a']*self.W[i,j] - self.kr[i,j,'i']*self.G[i,j]*self.G_scale)*self.PO[i,j]*self.PO_scale*self.Vi[i,j]*self.Vi_scale
#            else:
#                return Constraint.Skip
...
#            if j > 0:
#                return self.dX_dt[i, j] == \
#                       sum(self.ldot_t[j, k] * self.X[i, k] for k in self.cp)
#            else:
#                return Constraint.Skip
#        
...
#            if i < nfe and nfe > 1:
#                return self.X[i + 1, 0] - sum(self.l1_t[j] * self.X[i, j] for j in self.cp)
#            else:
#                return Expression.Skip
#        
...
#        
        ...
        def _ode_T(self,i,j):    
            if j > 0:
                ...
                return self.dT_dt[i,j] ==\
                            (self.Q_R[i,j] - self.Q_C[i,j] - self.F[i]*self.tf*self.monomer_cooling[i,j])/self.T_scale * self.Hrxn['p']/(self.m_tot[i,j]*self.m_tot_scale)/(self.bulk_cp_1 + self.bulk_cp_2*self.T[i,j]*self.T_scale)
            else:
                return Constraint.Skip
        ...
        def _Q_in(self,i,j):
            if j > 0: # normalized with delta_H_r
                return self.Q_R[i,j] == ((self.kr[i,j,'i']-self.kr[i,j,'p'])*(self.G[i,j]*self.G_scale + self.U[i,j]*self.U_scale) + (self.kr[i,j,'p'] + self.kr[i,j,'t'])*self.n_KOH + self.kr[i,j,'a']*self.W[i,j]*self.W_scale)*self.PO[i,j]*self.PO_scale*self.Vi[i,j]*self.Vi_scale*self.Hrxn_aux['p'] + self.dW_dt[i,j]*self.W_scale * self.Hrxn_aux['p']
            else:
                return Constraint.Skip
        ...
        def _rxn_rate_r_a(self,i,j,r):
            if j > 0:
                return 0.0 == (self.T[i,j]*self.T_scale*log(self.A[r]*self.scale*60*1000) - self.Ea[r]/self.Rg - self.T[i,j]*self.T_scale*self.k_l[i,j,r])
            else:
                return Constraint.Skip
        ...
        def _pc_T_max(self,i,j):
            if j > 0:
                return 0.0 == (self.T_safety + self.Tb) - self.T[i,j]*self.T_scale - self.s_T_max[i,j] + self.eps_pc[i,j,2]
            else:
                return Constraint.Skip
        ...
        def _pc_T_min(self,i,j):
            if j > 0:
                return 0.0 == self.T[i,j]*self.T_scale  - (100.0 + self.Tb)  - self.s_T_min[i,j] + self.eps_pc[i,j,3]
            else:
                return Constraint.Skip
        ...
        self.u1_e = Expression(self.fe_t, rule = lambda self, i: self.T_cw[i])
        ...
        # objective
        def _obj(self):
            return self.tf + self.rho*(sum(self.eps[i] for i in self.epc) + sum(sum(sum(self.eps_pc[i,j,k] for i in self.fe_t) for j in self.cp if j > 0) for k in self.pc)) #*nfe
    ...
    def create_output_relations(self):
        self.add_component('MW', Var(self.fe_t,self.cp, initialize=0, bounds=(0,None)))
        ...
        #for (i,j) in self.MW.index_set():
        #    if j > 0:
        #        self.MW[i,j].value = (self.MX[i,j,1].value/self.MX[i,j,0].value*self.MX1_scale/self.MX0_scale * self.mw_PO*self.num_OH + self.mw_PG)/self.MW_scale
        #    else:
        #        continue
...
#        self.pc_heat_removal_a.deactivate()
    ...
    def clear_bounds(self):
        ...
        variables = ['s_temp_b','s_T_min','s_T_max','s_heat_removal_a','s_mw','s_PO_ptg','s_unsat','s_mw','s_mw_ub','s_PO_fed','eps','eps_pc']
    ...
    def clear_aux_bounds(self):
        keep_bounds = ['s_temp_b','s_T_min','s_T_max','s_heat_removal_a','s_mw','s_PO_ptg','s_unsat','s_mw','s_mw_ub','s_PO_fed','eps','eps_pc','F','u1','u2','tf','T_cw','T'] 
        for var in self.component_objects(Var, active=True):
    ...
    def create_bounds(self):
        ...
        for i in self.fe_t:
            self.T_cw[i].setlb((50 + self.Tb)/self.T_scale)
            self.u1[i].setlb((50+self.Tb)/self.T_scale)
            self.T_cw[i].setub((170 + self.Tb)/self.T_scale)
            self.u1[i].setub((170 + self.Tb)/self.T_scale)
            self.F[i].setlb(0.0)
            self.u2[i].setlb(0.0)
            self.F[i].setub(3.0)
            self.u2[i].setub(3.0)
            for j in self.cp:
                self.T[i,j].setlb((80 + self.Tb)/self.T_scale) # loose bounds to robustify convergence
                self.T[i,j].setub((200 + self.Tb)/self.T_scale) # loose bounds to robustify convergence
    ...
    def del_pc_bounds(self):
        for i in self.fe_t:
            for j in self.cp:
                ...
                self.T[i,j].setlb(353.15/self.T_scale)
...
#        m_aux.pc_heat_removal_a.deactivate()
        ...
        m_aux.pc_T_min.deactivate()
        m_aux.F[1] = 1.0
        m_aux.T_cw[1] = (70.0+self.Tb.value)/self.T_scale
        m_aux.tf = min(8.0*24.0/self.nfe,8.0)
        m_aux.F[1].fixed = True
        m_aux.T_cw[1].fixed = True
        m_aux.tf.fixed = True
        #m_aux.T[1,0].fix(3.97)
        #m_aux.T[1,1].fix(3.97)
        #m_aux.T[1,2].fix(3.97)
        #m_aux.T[1,3].fix(3.97)
        #m_aux.de_T.deactivate()
        #m_aux.cp_T.deactivate()
        #m_aux.dvar_t_T.deactivate()
        #m_aux.T_icc.deactivate()
