...
#        self.scale = 1
        ...
        # heat transfer
        self.kA = Var(initialize=2200.0/self.Hrxn['p']*60.0/20.0) # /20 works fine, means following T_cw [kW/K]
        self.kA.fix()
...
#        self.epc_mw.deactivate()
    ...
    def par_to_var(self):
        ...
        self.A['i'].setub(self.A['i'].value*1.5) 
        self.A['p'].setlb(self.A['p'].value*0.5)
        self.A['p'].setub(self.A['p'].value*1.5)
        ...
        self.n_KOH.setub(1.5*self.n_KOH.value)
...
#        self.Prod_c.reconstruct()
    ...
    def clear_aux_bounds(self):
        keep_bounds = ['s_temp_b','s_T_min','s_T_max','s_mw','s_PO_ptg','s_unsat','s_mw','s_mw_ub','s_PO_fed','eps','eps_pc','u1','u2','tf','k_l','T','T_cw'] 
    ...
    def create_bounds(self):
        ...
        for i in self.fe_t:
            ...
            self.u2[i].setub(3.0)
            for j in self.cp:
                self.T_cw[i,j].setlb(273.15/self.T_scale)
                self.T_cw[i,j].setub((225.0 + self.Tb)/self.T_scale)
