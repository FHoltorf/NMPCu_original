...
class SemiBatchPolymerization_multistage(ConcreteModel):
    def __init__(self, nfe, ncp, **kwargs):   
        ...
        self.T_cw_ic = Param(initialize=373.15/self.T_scale, mutable=True) #373.15
...
#        self.epc_mw.deactivate() 
    ...
    def par_to_var(self):
        ...
        self.A['t'].setlb(self.A['t'].value*0.5)
        self.A['t'].setub(self.A['t'].value*2.0)
    ...
    def create_output_relations(self):
        ...
        for index in self.MW.index_set():
            ...
            s = index[2]
            try:
                self.MW[index].value = (self.MX[i,j,1,s].value*self.MX1_scale*self.mw_PO*self.num_OH/(self.MX[i,j,0,s].value*self.MX0_scale) + self.mw_PG)/self.MW_scale 
            except:
