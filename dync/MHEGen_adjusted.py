...
from main.dync.NMPCGen_adjusted import NmpcGen
import numpy as np
...
###############################################################################
        ...
        if self.multimodel or self.linapprox:
            for key in self.x_vars:
                ...
                self.x_vars[key] = deepcopy(aux)
            for key in self.y_vars:
                aux = []
                for element in self.y_vars[key]:    
                    aux.append(element + (1,))    
                self.y_vars[key] = deepcopy(aux)
    ...
    def create_mhe(self,initial_confidence=1e-6):
        ...
        self.lsmhe.Q_mhe = Param(range(0, self.nfe_mhe), self.lsmhe.xkNk_mhe, initialize=1.0, mutable=True) if self.diag_Q_R \
        ...
        if self.sens == 'sIpopt':
            self.lsmhe.yk0_mhe_var = Var(self.lsmhe.ykk_mhe, initialize=0.0) # declare variable for the last measurement
            self.lsmhe.artificial_constraint = Constraint(self.lsmhe.ykk_mhe, rule=lambda mod, j: mod.yk0_mhe_var[j] - mod.yk0_mhe[self.nfe_mhe,j] == 0.0)
        ...
        if self.sens == 'sIpopt':
            self.lsmhe.hyk_c_mhe = Constraint(self.lsmhe.fe_t, self.lsmhe.ykk_mhe,
                                          rule=
                                          lambda mod, t, i:mod.yk0_mhe[t, i] - self.yk_l[t][i] - mod.nuk_mhe[t, i] == 0.0 if i < self.nfe_mhe else \
                                          mod.yk0_mhe_var[i] - self.yk_l[t][i] - mod.nuk_mhe[t, i] == 0.0)
        else:
            self.lsmhe.hyk_c_mhe = Constraint(self.lsmhe.fe_t, self.lsmhe.ykk_mhe,
                                          rule=
        ...
        if self.noisy_inputs:
            for u in self.u:
                ...
                ce = getattr(self.lsmhe, u + "_e")  #: Get the expressions
    ...
    def solve_mhe(self, fix_noise = False):
        ...
        for x in self.states:
            for j in self.x_vars[x]:
                self.initial_values[(x,j)] = output[(x,(self.nfe_mhe,self.ncp_t)+j)]
...
###############################################################################
    ...
    def set_covariance_meas(self, cov_dict):
        ...
        for key in cov_dict:
            ...
            if aux_key[0] in self.yk_key: 
                ...
                for t in range(1,self.nfe_mhe+1):
                    if self.diag_Q_R:                
                        rtarget[t, v_i] = 1.0 / (max(abs(cov_dict[vni, vnj]*self.measurement[t][vni_m]),1e-4))**2
                    else:
                        # only allow for diagonal measurement covariance matrices 
                        rtarget[t, v_i, v_j] = 1.0 / (max(abs(cov_dict[vni, vnj]*self.measurement[t][vni_m]),1e-4))**2 ## fixx
            else:
                continue
    ...
    def set_covariance_disturb(self, cov_dict, set_bounds=True):
        ...
        for key in cov_dict[0]:
            ...
            if aux_key[0] in self.xkN_key:
                ...
                if self.diag_Q_R:
                    if xic_key == ():
                        qtarget[0, v_i] = 1 / (max(abs(cov_dict[0][vni, vnj]*xic.value),1e-4))**2 # .00001
                    else:
                        qtarget[0, v_i] = 1 / (max(abs(cov_dict[0][vni, vnj]*xic[xic_key].value),1e-4))**2 # .0001
                else:
                ...
                for t in range(1,self.nfe_mhe):
                    if self.diag_Q_R:
                        qtarget[t, v_i] = 1 / (max(abs(cov_dict[0][vni, vnj]*self.nmpc_trajectory[t,vni_traj]),1e-4))**2 
                        if set_bounds:                        
                            if cov_dict[0][vni, vnj] != 0.0:
    ...
    def set_covariance_u(self, cov_dict):
        ...
        for key in cov_dict:
            u = key[0]
            for t in range(1,self.nfe_mhe+1):
                utarget[t, u] = 1 / (max(abs(cov_dict[key]*self.nmpc_trajectory[t,u]),1e-4))**2
...
#                    var[(self.nfe_t, self.ncp_t) + j].set_suffix_value(self.lsmhe.dof_v, 1)
    ...
    def create_mhe_sIpopt_suffixes(self):
        m = self.lsmhe
    ...
    def mhe_sIpopt_update(self):
        for y in self.y:
            for jth in self.y_vars[y]:  #: the jth variable
                k = self.yk_key[(y, jth)]  #: The key needs to be created only once, that is why the loop was split
                self.lsmhe.sens_state_0.set_value(self.lsmhe.yk0_mhe_var[k], k)
                self.lsmhe.sens_state_1.set_value(self.lsmhe.yk0_mhe_var[k], k)
                self.lsmhe.sens_state_value_1.set_value(self.lsmhe.yk0_mhe_var[k], self.measurement[self.iterations][(y,jth)])
                self.lsmhe.sens_init_constr.set_value(self.lsmhe.hyk_c_mhe[self.nfe_mhe,k], k)
        ...
        results = sIP.solve(self.lsmhe, tee = True)
        
        self.lsmhe.solutions.load_from(results)
        self.nmpc_trajectory[self.iterations,'mhe_sIpopt'] = [str(results.solver.status),str(results.solver.termination_condition)]
                
        for x in self.states:
            xvar = getattr(self.lsmhe, x)
            for j in self.x_vars[x]:
                self.curr_estate[(x,j)] = self.lsmhe.sens_sol_state_1.get(xvar[(self.nfe_mhe,3)+j])
    
    def update_lsmhe(self):
        for y in self.y:
            for jth in self.y_vars[y]:  #: the jth variable
                k = self.yk_key[(y, jth)]
                self.lsmhe.yk0_mhe[self.nfe_mhe,k] = self.measurement[self.iterations][(y,jth)]
