...
ind = np.arange(len(folders)+2)
width = 1.0/8.0
bars = {}
xticks = []
xticks.append('open-loop' + '\n' + 'nominal')
for f in folders:
     xticks.append(method[f])
xticks.append('open-loop' + '\n' + 'multi-model')
...
k = 0
vio_nominal_openloop = {'epc_PO_ptg':130.51733684827855/scaling['epc_PO_ptg'],'epc_mw':3.7410094193048735/scaling['epc_mw'],'epc_unsat':0.013426473989343234/scaling['epc_unsat']}
for con in constraint_name:
    vals = []
    vals.append(vio_nominal_openloop[con])
    for folder in folders:
        vals.append(comparison[folder,con])
    vals.append(0)# open-loop control
...
vio_nominal_openloop = {'T_max':430.2956230944642-423.15,'T_min':0,'Tad_max':467.2020459298657-443.15}
k = 0
...
lgd = ax1.legend(loc ='lower center', ncol=3, fancybox=True, bbox_to_anchor=(0.5,-0.45))
ax2 = ax1.twinx()

color = ['k','k','k']
lstyle = ['-.','-','--']
#improvement = {'125grid/timeinvariant/parest/nominal':(-1.8,-5.4,-11.1),
# '125grid/timeinvariant/parest/ms':(-4.1,-12.8,-27.7),
# '125grid/timeinvariant/parest/SBSG':(-2.8,-15.7,-32.1),
# '125grid/timeinvariant/parest/nominal_bo':(-0.1,-23.2,-35.5)}
improvement = [(0,0,0),(-1.8,-5.4,-11.1),(-4.1,-12.8,-27.7),(-2.8,-15.7,-32.1),(-0.1,-23.2,-35.5),(0,0,0)]

k = 0
for t in ['max_tf','avg_tf','min_tf']:
    vals = []
    vals.append(305.5437805920627)
    for folder in folders:
        vals.append(comparison[folder,t])
    vals.append(582.8240007265179)# open-loop control

    ax2.plot(xspacing,vals,linestyle=lstyle[k],marker='o',color=color[k],label=labels[t])
    if p1 == '125grid/timeinvariant/parest/':
        for it in range(1,len(vals)-1):
            ax2.annotate(str(improvement[it][k])+'%',(xspacing[it],vals[it]),horizontalalignment='right',verticalalignment='bottom')
